...
const server = createServer((req, res) => {
  void (async () => {
    ...
    if (path === "/api/health") {
      json(res, 200, { ok: true, templates: templateMetas.length });
      return;
    }
    ...
    if (path === "/api/jobs" && req.method === "POST") {
      ...
      const meta = findTemplateMeta(body.templateId);
      if (!meta) {
        json(res, 400, { error: `لا يوجد قالب بالمعرّف ${body.templateId}` });
        return;
      }
      // القيم الافتراضية لا تُدمج هنا: Remotion يدمجها من defaultProps
      // المعرّفة في الـ Composition داخل الحزمة، فلا حاجة لتحميل الـ schema
      // في الخادم.
      const job = enqueue(meta.id, meta.nameAr, body.props ?? {});
