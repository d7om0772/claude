...
import type { PaperCardProps } from './schema';
import {
  FONT_STACK,
  FONT_WEIGHT_BLACK,
  FONT_WEIGHT_MEDIUM,
...
const applyKashida = (word: string): string => {
...
const kashidaLine = (line: string): string => {
  ...
  return words
    .map((w, i) => (i === bestIndex ? applyKashida(w) : w))
    .join(' ');
};

/** يختار الكابشن الموافق للفريم الحالي. */
const pickCaption = (
  captions: PaperCardProps['captions'],
  currentMs: number,
): PaperCardProps['captions'][number] | null => {
...
}> = ({ color, grainOpacity, lightIntensity }) => {
  return (
...
const DefaultMark: React.FC<{ size: number; color: string }> = ({
  size,
  color,
}) => {
...
const BrandLockup: React.FC<{
  ...
  scale: number;
}> = ({ logo, brandName, color, letterSpacing, scale }) => {
...
const CaptionLine: React.FC<{
  text: string;
  swashWordIndex: number;
  color: string;
  strokeColor: string;
  fontSize: number;
}> = ({ text, swashWordIndex, color, strokeColor, fontSize }) => {
  ...
  const approved = useMemo(
    () =>
      new Set(
        sanitizeSaltIndices(words, swashWordIndex >= 0 ? [swashWordIndex] : [], {
...
const TextScene: React.FC<{
  headline: PaperCardProps['headline'];
  subheadline?: string;
  cardColor: string;
  fontColor: string;
  opacity: number;
  scale: number;
}> = ({ headline, subheadline, cardColor, fontColor, opacity, scale }) => {
...
export const PaperCardTemplate: React.FC<PaperCardProps> = ({
  backgroundColor,
  fontColor,
  cardColor,
  brandColor,
  ...
  const progress = interpolate(frame, [slideStartFrame, slideEndFrame], [0, 1], {
  ...
  const offsets: Record<typeof slideDirection, [number, number]> = {
