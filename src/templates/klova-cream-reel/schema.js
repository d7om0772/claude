...
export const sceneSchema = z.object({
  ...
  /**
   * مدّة مكتوبة تتجاوز الاشتقاق التلقائي.
   *
   * الأصل أن المدّة تُشتقّ من الكلمات: اللقطة تبدأ مع أول كلمة فيها وتنتهي
   * قبل أول كلمة في التي تليها. وهذا يضبط التزامن مع الصوت، لكنه يسلب
   * المستخدم القدرة على إطالة لقطة أو تقصيرها لسبب بصري.
   *
   * فحين تُكتب هنا مدّة، تُستعمل كما هي ويمضي المؤشّر بها — فاللقطة التالية
   * تمتصّ الفرق تلقائياً لأن مدّتها مسافةٌ إلى كلمتها الأولى. والنتيجة:
   * الحدّ بين اللقطتين يتحرّك، ومجموع الفيديو لا يتغيّر، **وتوقيت الكلمات
   * لا يُمسّ إطلاقاً** فيبقى الكلام على الصوت.
   *
   * وقد تنتقل أسطر بين اللقطتين تبعاً للحدّ الجديد، وهو سلوك صحيح: السطر
   * يتبع اللقطة التي تحتوي بدايته.
   */
  durationOverrideInFrames: z
    .number()
    .int()
    .min(1)
    .nullable()
    .optional()
    .describe(
      "مدّة مكتوبة لهذه اللقطة بالفريمات تتجاوز الاشتقاق. الفارغ يعني الوقت التلقائي",
    ),
  media: z
    .string()
    .nullable()
  ...
  text: z
    ...
    .optional()
    .describe(
      "نص مشهدَي stack و echo — اتركه فارغاً: الفراغ هو الوضع الطبيعي، فيأخذ المشهد نصّه تلقائياً من أسطر الكابشن التي تبدأ في زمنه (وهي نفسها التي يعرضها محرّر اللقطات تحته)، فتعديل السكربت يغيّر ما يظهر هنا معه. اكتب نصّاً هنا فقط لتجاوز ذلك بنصّ ثابت لا يتبع السكربت",
    ),
  textYRatio: z
    .number()
    .min(0.02)
    ...
    .optional()
    .describe(
      "ارتفاع نصّ هذه اللقطة وحدها كنسبة من ارتفاع الإطار — الأكبر أنزل. يسري على ما تعرضه اللقطة: أسفلَ كتلة الكابشن في لقطات البطاقة والكريمي الفاضي، وأعلى الكلمات الضخمة في stack، ومركزَ البطاقة الملوّنة في echo. الفارغ يأخذ موضع القالب العام",
    ),
  clicks: z
    .boolean()
    .nullable()
