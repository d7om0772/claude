...
export const captionSchema = z.object({
  ...
  startMs: z.number().min(0).describe('لحظة ظهور الكابشن بالملي ثانية من بداية الفيديو'),
  endMs: z.number().min(0).describe('لحظة اختفاء الكابشن بالملي ثانية من بداية الفيديو'),
});
...
export const templateSchema = z.object({
  ...
  mediaVolume: z.number().min(0).max(1).describe('مستوى صوت المقطع نفسه — صفر يعني كتم'),

  /* ---------- الصوت والكابشن ---------- */
  voiceover: z
    .string()
    .optional()
    .describe('مسار التعليق الصوتي — لو وُجد فمدة الفيديو تُحسب من طوله تلقائياً'),
  ...
  revealDelayInFrames: z
    .number()
    .min(0)
    .describe('عدد الفريمات قبل أن يبدأ المقطع بالتمدد (وقفة تنفّس في البداية)'),
  revealDurationInFrames: z
    .number()
    .min(2)
    .describe('طول حركة التمدد بالفريمات — الأصل ٨ فريمات، كلما قلّت صارت أعنف'),
  revealDirection: z
    .enum(['vertical', 'horizontal'])
    .describe('اتجاه التمدد: vertical يفتح من شريط أفقي، horizontal يفتح من شريط عمودي'),
  ...
  revealAnchor: z
    ...
    .min(0)
    .max(1)
    .describe('نقطة الارتكاز من أعلى المقطع — ٠.٤٢ تجعل الحافة السفلية تنزل أكثر مما تصعد العلوية'),
  textDelayInFrames: z
    .number()
    .min(0)
