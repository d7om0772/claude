...
import { srtToCaptions } from "../lib/srt";
...
if (!input) {
  console.error("الاستخدام: npm run srt -- <ملف.srt> [<مخرج.json> | --props <props.json>]");
  process.exit(1);
}
...
if (second === "--props") {
  ...
  const props = JSON.parse(readFileSync(third, "utf8")) as Record<string, unknown>;
  writeFileSync(third, `${JSON.stringify({ ...props, captions }, null, 2)}\n`, "utf8");
